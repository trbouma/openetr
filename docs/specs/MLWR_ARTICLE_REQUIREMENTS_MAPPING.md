...
| Acknowledges receipt of goods | The current system treats the uploaded receipt document as event data and shows `Origin Event Data` for the signed object. The MLWR profile says goods and receipt terms remain part of the receipt representation or later structured payload. | Goods information is not a base protocol primitive; it is carried in the receipt data, referenced document, or future structured schema/attestation. | Partially supported as document evidence. Not yet implemented as structured goods fields, inspection evidence, quantity validation, or warehouse custody attestation. |
...
| Receipt object is tied to stored-goods documentation | The current flow accepts an uploaded warehouse receipt document such as `examples/MLWR001.pdf`, hashes it, and records event data including name, digest generation time, and size. | The document digest becomes the object identity used in `o` and `d` tags and query filters. | Partially aligned. The system identifies the receipt document but does not yet validate goods existence, quantity, custody, or conformity. |
